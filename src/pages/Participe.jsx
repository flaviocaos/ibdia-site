...
export default function Participe() {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <PageHero
        eyebrow="Colaboração"
        title="Participe do IBDIA"
        subtitle="Escolha a porta de entrada mais adequada para sua proposta. Cada submissão passa por triagem e avaliação institucional."
        centered
      />
...
<div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {options.map(({ icon: Icon, title, text, to, cta }) => (
          <div key={to} className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all p-8 flex flex-col">
            <div className="w-14 h-14 rounded-2xl bg-[#EAF4F6] text-[#073B67] flex items-center justify-center mb-6"><Icon size={28} /></div>
            ...
            <Link to={to} className="mt-7 inline-flex items-center gap-2 font-bold text-[#073B67] hover:text-[#07899A]">
